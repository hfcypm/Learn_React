...
1. 为订单查询建立复合索引，对比加索引前后的 EXPLAIN 结果。
2. 编写带 `FOR UPDATE` 的扣库存事务，模拟两个并发会话。
3. 故意制造一次锁等待，用 `pg_stat_activity` 观察并解除。
4. 设置 `ISOLATION LEVEL REPEATABLE READ`，验证快照读行为。
5. 运行 `ANALYZE` 后重测一次慢查询，记录执行计划变化。
